...
class TopicTypeNodeSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $group = $groupService->updateOrCreate('成人寫真-專題分類', TopicTypeNodeCodeConstants::PHOTOGRAPH_TOPIC_TYPE);
        if ($group) {
            ...
            $nodeRead = $nodeService->edit(
                $group,
                '成人寫真-專題分類-讀取',
                TopicTypeNodeCodeConstants::PHOTOGRAPH_TOPIC_TYPE_READ
            );
            if ($nodeRead) {
            ...
            $nodeCreate = $nodeService->edit(
                $group,
                '成人寫真-專題分類-新增',
                TopicTypeNodeCodeConstants::PHOTOGRAPH_TOPIC_TYPE_CREATE
            );
            if ($nodeCreate) {
            ...
            $nodeUpdate = $nodeService->edit(
                $group,
                '成人寫真-專題分類-編輯',
                TopicTypeNodeCodeConstants::PHOTOGRAPH_TOPIC_TYPE_UPDATE
            );
            if ($nodeUpdate) {
            ...
            $nodeDel = $nodeService->edit(
                $group,
                '成人寫真-專題分類-刪除',
                TopicTypeNodeCodeConstants::PHOTOGRAPH_TOPIC_TYPE_DELETE
            );
            if ($nodeDel) {
