...
class RouteServiceProvider extends ServiceProvider
{
    ...
    {
        $file = \Module::getModulePath('Advertisement') . '/Routes/web.php';
        if (file_exists($file)) {
    ...
    {
        $file = \Module::getModulePath('Advertisement') . '/Routes/api.php';
        if (file_exists($file)) {
            \Route::prefix('api')
                ->middleware('api')
                ->group($file);
    ...
    {
        $folder = \Module::getModulePath('Advertisement') . '/Routes/custom';
        if (\File::exists($folder)) {
            foreach (\File::files($folder) as $file) {
                \Route::as('advertisement_')->group($file);
            }
        }
    }
}
