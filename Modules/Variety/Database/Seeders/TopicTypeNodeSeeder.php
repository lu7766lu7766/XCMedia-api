...
class TopicTypeNodeSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $group = $groupService->updateOrCreate('影音綜藝-專題分類', TopicTypeNodeCodeConstants::VARIETY_TOPIC_TYPE);
        if ($group) {
            ...
            $nodeRead = $nodeService->edit(
                $group,
                '影音綜藝-專題分類-讀取',
                TopicTypeNodeCodeConstants::VARIETY_TOPIC_TYPE_READ
            );
            if ($nodeRead) {
            ...
            $nodeCreate = $nodeService->edit(
                $group,
                '影音綜藝-專題分類-新增',
                TopicTypeNodeCodeConstants::VARIETY_TOPIC_TYPE_CREATE
            );
            if ($nodeCreate) {
            ...
            $nodeUpdate = $nodeService->edit(
                $group,
                '影音綜藝-專題分類-編輯',
                TopicTypeNodeCodeConstants::VARIETY_TOPIC_TYPE_UPDATE
            );
            if ($nodeUpdate) {
            ...
            $nodeDel = $nodeService->edit(
                $group,
                '影音綜藝-專題分類-刪除',
                TopicTypeNodeCodeConstants::VARIETY_TOPIC_TYPE_DELETE
            );
            if ($nodeDel) {
