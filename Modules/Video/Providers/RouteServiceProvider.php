...
class RouteServiceProvider extends ServiceProvider
{
    ...
    {
        $file = \Module::getModulePath('Video') . '/Routes/web.php';
        if (file_exists($file)) {
    ...
    {
        $file = \Module::getModulePath('Video') . '/Routes/api.php';
        if (file_exists($file)) {
            \Route::prefix('api')
                ->middleware('api')
                ->group($file);
    ...
    {
        $folder = \Module::getModulePath('Video') . '/Routes/custom';
        if (\File::exists($folder)) {
            foreach (\File::files($folder) as $file) {
                \Route::as('video_')->group($file);
            }
        }
    }
}
