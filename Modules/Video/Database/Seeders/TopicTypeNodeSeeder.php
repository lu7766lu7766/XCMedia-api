...
class TopicTypeNodeSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $group = $groupService->updateOrCreate('成人視頻-專題分類', TopicTypeNodeCodeConstants::VIDEO_TOPIC_TYPE);
        if ($group) {
            ...
            $nodeRead = $nodeService->edit(
                $group,
                '成人視頻-專題分類-讀取',
                TopicTypeNodeCodeConstants::VIDEO_TOPIC_TYPE_READ
            );
            if ($nodeRead) {
            ...
            $nodeCreate = $nodeService->edit(
                $group,
                '成人視頻-專題分類-新增',
                TopicTypeNodeCodeConstants::VIDEO_TOPIC_TYPE_CREATE
            );
            if ($nodeCreate) {
            ...
            $nodeUpdate = $nodeService->edit(
                $group,
                '成人視頻-專題分類-編輯',
                TopicTypeNodeCodeConstants::VIDEO_TOPIC_TYPE_UPDATE
            );
            if ($nodeUpdate) {
            ...
            $nodeDel = $nodeService->edit(
                $group,
                '成人視頻-專題分類-刪除',
                TopicTypeNodeCodeConstants::VIDEO_TOPIC_TYPE_DELETE
            );
            if ($nodeDel) {
