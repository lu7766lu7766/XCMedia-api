...
class TopicTypeNodeSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $group = $groupService->updateOrCreate('影音電影-專題分類', TopicTypeNodeCodeConstants::MOVIE_TOPIC_TYPE);
        if ($group) {
            ...
            $nodeRead = $nodeService->edit(
                $group,
                '影音電影-專題分類-讀取',
                TopicTypeNodeCodeConstants::MOVIE_TOPIC_TYPE_READ
            );
            if ($nodeRead) {
            ...
            $nodeCreate = $nodeService->edit(
                $group,
                '影音電影-專題分類-新增',
                TopicTypeNodeCodeConstants::MOVIE_TOPIC_TYPE_CREATE
            );
            if ($nodeCreate) {
            ...
            $nodeUpdate = $nodeService->edit(
                $group,
                '影音電影-專題分類-編輯',
                TopicTypeNodeCodeConstants::MOVIE_TOPIC_TYPE_UPDATE
            );
            if ($nodeUpdate) {
            ...
            $nodeDel = $nodeService->edit(
                $group,
                '影音電影-專題分類-刪除',
                TopicTypeNodeCodeConstants::MOVIE_TOPIC_TYPE_DELETE
            );
            if ($nodeDel) {
