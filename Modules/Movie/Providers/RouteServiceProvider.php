...
class RouteServiceProvider extends ServiceProvider
{
    ...
    {
        $file = \Module::getModulePath('Movie') . '/Routes/web.php';
        if (file_exists($file)) {
    ...
    {
        $file = \Module::getModulePath('Movie') . '/Routes/api.php';
        if (file_exists($file)) {
            \Route::prefix('api')
                ->middleware('api')
                ->group($file);
    ...
    {
        $folder = \Module::getModulePath('Movie') . '/Routes/custom';
        if (\File::exists($folder)) {
            foreach (\File::files($folder) as $file) {
                \Route::as('movie_')->group($file);
            }
        }
    }
}
