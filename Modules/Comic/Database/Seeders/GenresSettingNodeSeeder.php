...
class GenresSettingNodeSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $group = $groupService->updateOrCreate(
            '成人漫畫-類型設定',
            GenresSettingNodeCodeConstants::COMIC_GENRES_SETTING
        );
        if ($group) {
            ...
            $nodeRead = $nodeService->edit(
                $group,
                '成人漫畫-類型設定-讀取',
                GenresSettingNodeCodeConstants::COMIC_GENRES_SETTING_READ
            );
            if ($nodeRead) {
            ...
            $nodeCreate = $nodeService->edit(
                $group,
                '成人漫畫-類型設定-新增',
                GenresSettingNodeCodeConstants::COMIC_GENRES_SETTING_CREATE
            );
            if ($nodeCreate) {
            ...
            $nodeUpdate = $nodeService->edit(
                $group,
                '成人漫畫-類型設定-編輯',
                GenresSettingNodeCodeConstants::COMIC_GENRES_SETTING_UPDATE
            );
            if ($nodeUpdate) {
            ...
            $nodeDel = $nodeService->edit(
                $group,
                '成人漫畫-類型設定-刪除',
                GenresSettingNodeCodeConstants::COMIC_GENRES_SETTING_DELETE
            );
            if ($nodeDel) {
