...
class SourceSettingNodeSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $group = $groupService->updateOrCreate('影音戲劇-來源設定', SourceSettingNodeCodeConstants::DRAMA_SOURCE_SETTING);
        if ($group) {
            ...
            $nodeRead = $nodeService->edit(
                $group,
                '影音戲劇-來源設定-讀取',
                SourceSettingNodeCodeConstants::DRAMA_SOURCE_SETTING_READ
            );
            if ($nodeRead) {
            ...
            $nodeCreate = $nodeService->edit(
                $group,
                '影音戲劇-來源設定-新增',
                SourceSettingNodeCodeConstants::DRAMA_SOURCE_SETTING_CREATE
            );
            if ($nodeCreate) {
            ...
            $nodeUpdate = $nodeService->edit(
                $group,
                '影音戲劇-來源設定-編輯',
                SourceSettingNodeCodeConstants::DRAMA_SOURCE_SETTING_UPDATE
            );
            if ($nodeUpdate) {
            ...
            $nodeDel = $nodeService->edit(
                $group,
                '影音戲劇-來源設定-刪除',
                SourceSettingNodeCodeConstants::DRAMA_SOURCE_SETTING_DELETE
            );
            if ($nodeDel) {
