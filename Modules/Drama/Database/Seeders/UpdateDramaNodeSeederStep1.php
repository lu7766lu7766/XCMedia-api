...
class UpdateDramaNodeSeederStep1 extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        $group = $groupService->updateOrCreate('电视管理', ManageDramaNodeCodeConstants::MANAGE_DRAMA);
        if ($group) {
        ...
        //集數設定
        $groupService = ManageNodeGroupService::getInstance();
        $group = $groupService->updateOrCreate('电视管理-集数设定', ManageDramaNodeCodeConstants::MANAGE_DRAMA_EPISODE);
        if ($group) {
            $roleService = new AdminRoleService();
            $nodeService = ManageNodeService::getInstance();
            $nodeRead = $nodeService->edit(
